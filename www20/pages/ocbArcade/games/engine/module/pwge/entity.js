    ...
    Entity.prototype._flush = function() {
        ...
        if (data.image && data.sw > 0 && data.sh > 0) {
            ...
            if(!!this.domRendering) {
                ...
                tmp = '-webkit-transform: translate3d(' + data.dx*renderer.owner.xScale + 'px,' + data.dy*renderer.owner.yScale + 'px,0) ';
                tmp += 'scale('+ ws + "," + hs + ')';
            ...
            if(!!this.domRendering) {
                ...
                tmp = '-webkit-transform: translate3d(' + this.x * renderer.owner.xScale + 'px,' + this.y * renderer.owner.xScale + 'px,0) ';
                tmp += 'scale(' + renderer.owner.xScale + "," + renderer.owner.yScale + ')';
                // tmp = 'translate3d(' + this.x * renderer.owner.xScale + 'px,' + this.y * renderer.owner.xScale + 'px,0) ';
                // tmp += 'scale(' + renderer.owner.xScale + "," + renderer.owner.yScale + ')';

                this.scaledPosition = {
                    x: this.x * renderer.owner.xScale,
                    y: this.y * renderer.owner.yScale,
                    width: this.width * renderer.owner.xScale,
                    height: this.height * renderer.owner.yScale
                }
                // this.domRenderer.setCSSProperty({key:"webkitTransform",value:tmp});
                this.domRenderer.draw(tmp);
            }
        }
